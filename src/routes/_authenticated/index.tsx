...
import { Plus, Search, Sparkles, Globe, Mail, Calendar, MessageSquare, Zap, Bot, Brain, Shield, ArrowRight, ChevronRight, Mic, Video, TriangleAlert as AlertTriangle, CircleUser as UserCircle } from "lucide-react";
...
const QUICK_ACTIONS = [
  ...
  },
  {
    ...
    prompt: "Research my top 3 competitors and draft a one-page brief",
  },
  {
    label: "Email Draft",
    icon: Mail,
    color: "bg-rose-500/20 text-rose-400",
    prompt: "Summarize my unread email and draft replies",
  },
  {
    label: "Schedule",
    icon: Calendar,
    color: "bg-cyan-500/20 text-cyan-400",
    prompt: "Find the best meeting slot this week",
  },
  {
    label: "Message",
    icon: MessageSquare,
    color: "bg-green-500/20 text-green-400",
    prompt: "Send a message via iMessage or WhatsApp",
  },
  {
    label: "Workflow",
    icon: Zap,
    color: "bg-amber-500/20 text-amber-400",
    prompt: "Build me a custom skill: summarise my week every Friday",
  },
];
...
const RECENT_ACTIVITY = [
  ...
  {
    label: "Scheduled standup for tomorrow",
    time: "15m ago",
    icon: Calendar,
    color: "text-cyan-400",
  },
  ...
  {
    label: "Saved tone preference to memory",
    time: "3h ago",
    icon: Brain,
    color: "text-amber-400",
  },
  {
    label: "Privacy-routed 3 emails on-device",
    time: "3h ago",
    icon: Shield,
    ...
    <div className="px-5 pt-14 pb-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <h1 className="text-3xl font-semibold tracking-tight mt-0.5">
            {getGreeting(activePersona?.profile.name)}
          </h1>
        </div>
        <div className="flex items-center gap-2">
          <NotificationCenter />
      ...
      >
        ...
        <div className="relative">
          ...
          <h2 className="text-xl font-semibold leading-tight">
            Ask anything.
            <br />
            Sage routes to the right model.
          </h2>
          <p className="text-sm text-muted-foreground mt-1.5">
            Sub-agents work in the background while you keep moving.
          </p>
      ...
      >
        ...
        <div className="flex-1 text-left">
          ...
          <p className="text-[11px] text-muted-foreground mt-0.5">
            Tap to hear your day — calendar, email, news
          </p>
        </div>
        <div className="flex gap-0.5 items-end h-5 shrink-0">
          {[3, 5, 7, 4, 6, 3, 5].map((h, i) => (
            <div
              key={i}
              className="w-0.5 bg-rose-400/50 rounded-full"
      ...
      <div className="mb-5">
        ...
        <div className="space-y-2">
          {(isDemoMode
            ? (activePersona?.agents ?? []).slice(0, 2).map((a) => ({
                name: a.name,
                status: a.status,
                model: a.model,
                progress: a.progress,
            ...
            <Link
              key={a.name}
              to="/agents"
              className="block glass rounded-2xl p-3 active:scale-[0.99] transition-transform"
            >
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-xl glass flex items-center justify-center shrink-0">
                  <Bot
                    className={`w-4 h-4 ${a.status === "running" ? "text-primary" : "text-amber-400"}`}
                  />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <div
                        className={`w-1.5 h-1.5 rounded-full ${a.status === "running" ? "bg-emerald-400 animate-pulse" : "bg-amber-400"}`}
                        aria-hidden="true"
                      />
                      <span className="sr-only">
                        {a.status === "running" ? "Running" : "Scheduled"}.
                      </span>
        ...
        <div className="glass rounded-2xl overflow-hidden divide-y divide-white/5">
          {RECENT_ACTIVITY.map((a, i) => (
            <div key={i} className="flex items-center gap-3 px-3 py-2.5">
              <div
                className={`w-7 h-7 rounded-lg glass flex items-center justify-center shrink-0 ${a.color}`}
              >
      ...
      {isError && (
        <div className="glass rounded-2xl p-4 flex items-center gap-3 text-sm mb-3">
          ...
          <button onClick={() => refetch()} className="text-primary text-xs font-medium">
            Retry
          </button>
        </div>
      )}
        ...
        {visibleThreads.map((t) => (
          <li key={t.id}>
            ...
            >
              ...
              {t.preview && (
                <p className="text-sm text-muted-foreground truncate mt-1 pl-4">{t.preview}</p>
              )}
            </Link>
          </li>
        ))}
