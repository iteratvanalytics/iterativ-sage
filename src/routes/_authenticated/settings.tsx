...
const MODELS = [
  { id: "auto", label: "Auto-route", sub: "Sage picks the best model per task", recommended: true },
  {
    ...
    sub: "Best for drafting, email, complex reasoning",
    recommended: false,
  },
  {
    id: "gemini25",
    label: "Gemini 2.5 Pro",
    sub: "Best for code, long context, vision",
    recommended: false,
  },
  {
    id: "gemini3",
    label: "Gemini 3 Flash",
    sub: "Fastest for simple, quick responses",
    recommended: false,
  },
...
const TOGGLE_GROUPS: { label: string; toggles: ToggleDef[] }[] = [
  {
    ...
    toggles: [
      { id: "voice", label: "Voice input", sub: "Tap mic to dictate messages", default: true },
      {
        ...
        sub: "Words appear as Sage types them",
        default: true,
      },
      {
        id: "autosuggest",
        label: "Auto-suggest chips",
        sub: "Show action chips after each response",
        default: true,
      },
      {
        id: "citations",
        label: "Research citations",
        sub: "Show source links in research responses",
        default: true,
      },
      {
        id: "reasoning",
        label: "Reasoning chain",
        sub: "Expand Sage's step-by-step thinking",
        default: false,
      },
    ],
  },
    ...
    toggles: [
      {
        ...
        sub: "Auto-update memory from conversations",
        default: true,
      },
      {
        id: "autoagent",
        label: "Background agents",
        sub: "Run scheduled workflows even when app is closed",
        default: true,
      },
      {
        id: "notifs",
        label: "Agent notifications",
        sub: "Notify when background tasks complete",
        default: true,
      },
    ],
  },
    ...
    toggles: [
      {
        id: "safebrowse",
        label: "Safe browsing",
        sub: "Filter unsafe URLs in research results",
        default: true,
      },
      {
        id: "experimental",
        label: "Experimental features",
        sub: "Early access to features in development",
        default: false,
      },
    ],
  },
...
const CONSENT_LOG = [
  ...
  {
    action: "Inbox triage — on-device classify",
    channel: "On-device",
    time: "3h ago",
    allowed: true,
  },
...
const MORE_ROWS = [
  { icon: Star, label: "Rate Sage", action: () => toast.success("Thank you! ⭐") },
  {
    icon: HelpCircle,
    label: "Help & support",
    action: () => toast.info("Support centre — coming soon"),
  },
  ...
  { icon: Globe, label: "Open in browser", action: () => toast.info("sage.app/app") },
  {
    ...
    action: () => toast.error("Contact support to delete your account"),
  },
];
    ...
    <div className="px-5 pt-14 pb-8">
      ...
      <div className="glass-strong rounded-3xl p-5 mt-6 flex items-center gap-4">
        <div className="relative">
          <div
            className="w-14 h-14 rounded-full flex items-center justify-center text-primary shadow-[var(--shadow-glow)]"
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            <p className="font-semibold truncate">
              {isDemoMode && activePersona ? activePersona.profile.name : "Dev User"}
            </p>
            <span
              className="text-[9px] px-1.5 py-0.5 rounded-full font-semibold text-primary-foreground"
              style={{ background: "var(--gradient-hero)" }}
            >
              {isDemoMode && activePersona ? activePersona.profile.plan : "PRO"}
            </span>
          </div>
          <p className="text-xs text-muted-foreground">
            {isDemoMode && activePersona
              ? activePersona.profile.role
              : "Unlimited · Multi-model · Background agents"}
          </p>
        ...
        {isDemoMode && (
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              className="text-[10px] px-2 py-1 rounded-full bg-primary/20 text-primary hover:bg-primary/30 transition-colors"
            >
              Switch
            </button>
            <button
              ...
              className="text-[10px] px-2 py-1 rounded-full glass text-muted-foreground hover:text-foreground transition-colors"
            >
              Exit
            </button>
          </div>
          ...
          {MODELS.map((m) => (
            <button
              ...
              onClick={() => {
                ...
                toast.success(`Default model → ${m.label}`);
              }}
              className="w-full flex items-center gap-3 px-4 py-3 active:bg-white/5 transition-colors"
            >
              <div
                className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${selectedModel === m.id ? "bg-primary/20" : "glass"}`}
              >
                <Cpu
                  className={`w-4 h-4 ${selectedModel === m.id ? "text-primary" : "text-muted-foreground"}`}
                />
              </div>
              <div className="flex-1 text-left min-w-0">
                <div className="flex items-center gap-1.5">
                  <span
                    className={`text-sm font-medium ${selectedModel === m.id ? "text-foreground" : "text-muted-foreground"}`}
                  >
                    {m.label}
                  </span>
                  {m.recommended && (
              ...
              return (
                <div key={t.id} className="flex items-center gap-3 px-4 py-3.5">
                  <div className="w-8 h-8 rounded-lg glass flex items-center justify-center shrink-0">
                    <Icon
                      className={`w-4 h-4 ${toggles[t.id] ? "text-primary" : "text-muted-foreground"}`}
                    />
                  </div>
                  <div className="flex-1 min-w-0">
      ...
      <div className="mt-6">
        ...
        <div className="glass rounded-2xl overflow-hidden divide-y divide-white/5">
          {[
            ...
            <button
              key={r.label}
              onClick={() => toast.info(`${r.label} settings — coming soon`)}
        ...
        {showConsent && (
          <div className="glass rounded-2xl overflow-hidden divide-y divide-white/5 animate-in fade-in slide-in-from-top-2 duration-200">
              ...
              <div key={i} className="flex items-center gap-3 px-4 py-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    {entry.channel} · {entry.time}
                  </p>
                </div>
                <span
                  className={`text-[10px] shrink-0 ${entry.allowed ? "text-emerald-400" : "text-amber-400"}`}
                >
            ...
            <div className="px-4 py-3 text-center">
              <p className="text-[10px] text-muted-foreground">
                Audit log kept locally · never uploaded
              </p>
            </div>
          </div>
        )}
              ...
              <div key={i} className="flex items-center justify-between px-4 py-3">
                ...
                <div className="flex items-center gap-1">
                  {s.keys.map((k, j) => (
                    <kbd
                      key={j}
                      className="px-1.5 py-0.5 rounded-md glass text-[11px] font-mono text-muted-foreground"
                    >
                      {k}
                    </kbd>
                  ))}
      ...
      <div className="mt-8 text-center space-y-1">
        <div className="flex items-center justify-center gap-2 mb-2">
          <div
            className="w-6 h-6 rounded-full flex items-center justify-center text-primary"
            style={{ background: "var(--gradient-orb)" }}
          >
        ...
        <p className="text-[10px] text-muted-foreground/50">
          Multi-model · Raft network · Background agents
        </p>
        <p className="text-[10px] text-muted-foreground/50">
          On-device privacy · Consent ledger · POPIA/GDPR
        </p>
      </div>
