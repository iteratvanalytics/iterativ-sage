...
const CATEGORY_META: Record<string, { color: string; icon: typeof Brain; label: string }> = {
  general:     { color: "text-primary",      icon: Brain,     label: "General"     },
  preferences: { color: "text-amber-400",    icon: Settings,  label: "Preferences" },
  people:      { color: "text-rose-400",     icon: User,      label: "People"      },
  projects:    { color: "text-emerald-400",  icon: Briefcase, label: "Projects"    },
  workflows:   { color: "text-cyan-400",     icon: Zap,       label: "Workflows"   },
  knowledge:   { color: "text-fuchsia-400",  icon: BookOpen,  label: "Knowledge"   },
};
...
function MemoryPage() {
  ...
  const exportData = () => {
    ...
    a.href = url;
    a.download = `sage-memory-export-${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
  ...
  const totalUsed = memories.length;
  const totalBudget = 200;
  const usagePct = Math.min((totalUsed / totalBudget) * 100, 100);
  ...
  return (
    <div className="px-5 pt-14 pb-8">
      ...
      <p className="text-sm text-muted-foreground mt-2">What Sage remembers about you across every session.</p>

      {/* Privacy note */}
      <div className="mt-4 flex items-start gap-2 px-3 py-2.5 rounded-xl bg-emerald-500/10 text-emerald-400/90 text-[11px]">
        <Shield className="w-3.5 h-3.5 shrink-0 mt-0.5" />
        <span>Memory is stored locally and encrypted on-device. Deletion is real deletion — no shadow copy retained.</span>
      </div>
      ...
      <div className="flex gap-3 mt-5">
        ...
        <div className="flex-1 glass rounded-2xl p-3 text-center">
          <p className="text-2xl font-bold text-amber-400">{totalBudget - totalUsed}</p>
          <p className="text-[10px] text-muted-foreground uppercase tracking-wider mt-1">Budget left</p>
        </div>
      </div>
      ...
      <div className="mt-3">
        <div className="flex items-center justify-between mb-1">
          ...
          <span className="text-[10px] text-muted-foreground">{totalUsed}/{totalBudget}</span>
        </div>
      ...
      <form
        ...
        className="glass-strong rounded-2xl p-3 mt-6 space-y-2"
      >
        <Input
          value={content}
      ...
      <div className="flex gap-1.5 overflow-x-auto hide-scrollbar pb-2">
        ...
        {CATEGORIES.map(c => (
          stats[c] ? (
            ...
            >
              <div className={`w-1.5 h-1.5 rounded-full ${CATEGORY_META[c].color.replace("text-", "bg-")}`} />
          ...
          <div className="text-center py-12">
            ...
            {!search && memories.length === 0 && (
              <button
                ...
                className="mt-4 px-4 py-2 glass rounded-xl text-sm text-muted-foreground hover:text-foreground transition-colors"
              >
        ...
        <div className="mt-6 space-y-2">
          ...
          {!showEraseConfirm ? (
            ...
            <div className="rounded-2xl border border-destructive/30 bg-destructive/5 p-3 space-y-2">
              ...
              <div className="flex gap-2">
                <button onClick={() => setShowEraseConfirm(false)} className="flex-1 glass rounded-xl py-2 text-sm text-muted-foreground">Cancel</button>
                <button
                  onClick={async () => {
