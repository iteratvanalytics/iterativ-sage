...
type SubAgent = { name: string; model: string; goal: string; status: "done" | "running"; duration?: string };
...
const INITIAL_RUNS: AgentRun[] = [
  {
    id: "1", name: "Inbox Triage", goal: "Sort & label every email since 7am, draft priority replies",
    status: "running", eta: "2m", model: "Claude Sonnet 4", progress: 67,
    lastOutput: "Processed 34 emails · 12 drafted · 3 flagged for review",
    privacy: "on-device",
    subagents: [
      { name: "Classifier",   model: "On-device",       goal: "Classify by urgency & sender",  status: "done",    duration: "1.2s" },
      { name: "Drafter",      model: "Claude Sonnet 4", goal: "Draft priority replies",        status: "running" },
    ],
  },
  {
    id: "2", name: "Competitor Watch", goal: "Daily summary of 8 product sites, pricing alerts",
    status: "scheduled", eta: "tomorrow 8am", model: "Perplexity Sonar", progress: 0,
    lastOutput: "Scheduled · will run tomorrow at 08:00",
    privacy: "cloud-with-consent",
    subagents: [
      { name: "Scout A",   model: "Perplexity Sonar", goal: "Scrape 4 product sites",      status: "done" },
      { name: "Scout B",   model: "Perplexity Sonar", goal: "Scrape 4 pricing pages",      status: "done" },
      { name: "Analyst",   model: "o3",               goal: "Synthesise deltas + alerts",  status: "done" },
    ],
  },
  {
    id: "3", name: "Slide Deck — Q3 Review", goal: "Draft 12 slides from notes + last quarter's data",
    status: "running", eta: "5m", model: "Gemini 2.5 Pro", progress: 45,
    lastOutput: "Slide 6 of 12 · Charts rendering",
    privacy: "cloud-with-consent",
    subagents: [
      { name: "Researcher",  model: "Perplexity Sonar", goal: "Pull Q3 metrics",              status: "done",    duration: "2.1s" },
      { name: "Designer",    model: "Gemini 2.5 Pro",   goal: "Draft slide layouts",          status: "running" },
      { name: "Chart Maker", model: "Gemini 2.5 Pro",   goal: "Render data visualisations",  status: "running" },
      { name: "Writer",      model: "Claude Sonnet 4",  goal: "Write speaker notes",          status: "running" },
    ],
  },
  {
    id: "4", name: "Resume Calls", goal: "Find unmet contacts in CRM and draft outreach sequences",
    status: "done", eta: "done · 14 min ago", model: "o3", progress: 100,
    lastOutput: "23 sequences drafted · 8 contacts prioritised",
    privacy: "cloud-with-consent",
    subagents: [
      { name: "CRM Scout",  model: "o3-mini",         goal: "Find lapsed contacts",       status: "done", duration: "3.4s" },
      { name: "Writer",     model: "Claude Sonnet 4", goal: "Draft outreach sequences",   status: "done", duration: "8.2s" },
    ],
  },
  {
    id: "5", name: "Travel Planner", goal: "2-day Nairobi trip around Thursday meeting",
    status: "done", eta: "done · 1h ago", model: "o3", progress: 100,
    lastOutput: "3 flights shortlisted · Itinerary ready · 2 hotels compared",
    privacy: "cloud-with-consent",
    subagents: [
      { name: "Flight Scout", model: "Perplexity Sonar", goal: "Find NBO flights",           status: "done", duration: "4.2s" },
      { name: "Hotel Scout",  model: "Perplexity Sonar", goal: "Compare hotels near CBD",    status: "done", duration: "3.8s" },
      { name: "Writer",       model: "Claude Sonnet 4",  goal: "Assemble itinerary",         status: "done", duration: "2.1s" },
    ],
  },
  {
    id: "6", name: "Code Review Bot", goal: "Review all open PRs, flag regressions, suggest fixes",
    status: "paused", eta: "paused", model: "Gemini 2.5 Pro", progress: 30,
...
const WORKFLOW_TEMPLATES = [
  ...
  { icon: Shield,   label: "Privacy Audit",          prompt: "Audit all skills for their current data access permissions" },
];
...
function AgentsPage() {
  ...
  const addRun = (template: typeof WORKFLOW_TEMPLATES[0]) => {
    const newRun: AgentRun = {
      ...
      progress: 0,
      subagents: [{ name: "Planner", model: "o3-mini", goal: "Decompose task into sub-steps", status: "running" }],
      privacy: "cloud-with-consent",
    };
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-2 flex-wrap">
                      ...
                      <span className="text-[10px] text-muted-foreground">{r.subagents.length} sub-agents</span>
                      <span className="text-[10px] text-muted-foreground">{r.model}</span>
                      {r.privacy && (
                        <span className={`text-[10px] flex items-center gap-0.5 ${r.privacy === "on-device" ? "text-emerald-400" : "text-amber-400/80"}`}>
                    ...
                    {r.subagents.map((sa, i) => (
                      <div key={i} className="flex items-center gap-3 px-2 py-2 rounded-xl glass">
                        ...
                        <div className="flex items-center gap-1.5 shrink-0">
                          {sa.duration && <span className="text-[10px] text-muted-foreground">{sa.duration}</span>}
                          {sa.status === "done"
          ...
          <div className="relative w-full max-w-[480px] glass-strong rounded-t-3xl p-5 pb-10 animate-in slide-in-from-bottom-4">
            <div className="flex items-center justify-between mb-5">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">Choose a template or describe your own</p>
              </div>
              <button
